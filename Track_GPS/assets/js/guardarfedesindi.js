      ...
      .then(data => {
        if(data.success) {
          ...
          var modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('modalFederacion'));
          modal.hide();
        } else {
      ...
      .then(data => {
        if(data.success) {
          ...
          if (!noti) {
            ...
            noti.style.display = 'block';
            noti.style.zIndex = '9999';
            noti.innerHTML = `<div class="modal-dialog modal-dialog-centered"><div class="modal-content bg-success text-white text-center" style="border-radius: 1rem; box-shadow: 0 0 20px #0002;"><div class="modal-body"><span style="font-size:1.2rem;">Sindicato guardado correctamente</span></div></div></div>`;
            document.body.appendChild(noti);
          } else {
            noti.classList.add('show');
            noti.style.display = 'block';
