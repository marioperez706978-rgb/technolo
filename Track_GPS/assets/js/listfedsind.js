        ...
        if (data.success && data.usuario) {
          ...
          if (data.vehiculos && data.vehiculos.length > 0) {
            html += '<div class="table-responsive"><table class="table table-bordered table-striped"><thead><tr>' +
              '<th>Tipo</th><th>Marca</th><th>Año</th><th>Placa</th><th>Federación</th><th>Sindicato</th></tr></thead><tbody>';
            data.vehiculos.forEach(vehiculo => {
              html += `<tr><td>${vehiculo.tipoV}</td><td>${vehiculo.marcaV}</td><td>${vehiculo.anioV}</td><td>${vehiculo.placa}</td><td>${vehiculo.federacion || ''}</td><td>${vehiculo.sindicato || ''}</td></tr>`;
            });
